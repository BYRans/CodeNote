...
// 过滤新疆与国外通话的数据。即通话一方为新疆归属地，另一方号码开头为00
// 并增加新疆号码字段和国外号码字段
// 下面语句没有加时间范围条件，这里要加近一周，分析近一周的数据
// 去除重点人的通话数据。如果包含重点人的数据，有可能挖掘到的疑似人本来就是重点人
// 这里去除的方法为：先取全集，再从全集中去除是重点人的通话数据。这么做的原因是，在hive sql中不能用“字段A!=字段B”的where条件，“=”其实是join条件的缩写，如果用“!=”其实是指join条件为不等于
...
// 过滤与关注国家的通话，并按新疆号码分组统计。注意：当前没有统按国家分别进行统计
...
// 将疑似人与重点人通话数据，按疑似人分组统计通话次数并排序,获取前20个为疑似人
// 注意，此时可能不够20个，即与重点人有联系的嫌疑人不足20个，所以要从原来的嫌疑人中补齐20个
...
// 生成疑似重点人表，最频繁国家暂时为空(全设为国外)，最频繁联系的重点人为空
